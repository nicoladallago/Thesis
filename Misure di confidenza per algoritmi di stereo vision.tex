	...
	\pagestyle{fancy}
		...
		\label{sec:localProperties}	
			...
			\label{par:local}
				...
				\begin{equation}
					C_{LC}=\frac {max\{ c(d_{1}-1) ,c(d_{1}+1) \} - c_{1}} {\gamma}
					\label{eq:LC}
				\end{equation}
		...
		\label{sec:leftRight}
			...
			\label{par:LRC}
				...
				\begin{equation}
					C_{LRC}(x,y) = - \abs{ d_{1}-D_{R}(x-d_{1},y)   }
					\label{eq:LRC}
				\end{equation}

				\noindent dove $D_{R}(x-d_{1},y)=argmin_{d_{R}}\{c_{R}(x-d_{1},y,d_{R})\}$ e ovviamente $d_{1}=argmin_{d}\{c(x,y,d)\}$
